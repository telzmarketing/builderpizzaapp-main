...
-- desde que os 6 produtos abaixo existam na tabela products.
-- Nao sobrescreve registros existentes e evita duplicar a mesma combinacao.
-- A data e usada apenas para agenda/gestao interna; a mensagem publica nao exibe tempo.
...
base_rows AS (
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE candidate.active = TRUE
      AND (
        lower(candidate.name) = lower(ps.product_name)
        OR lower('Pizza ' || candidate.name) = lower(ps.product_name)
        OR lower(candidate.name) = lower(replace(ps.product_name, 'Pizza ', ''))
      )
    ORDER BY
      CASE WHEN lower(candidate.name) = lower(ps.product_name) THEN 0 ELSE 1 END,
      candidate.name
    LIMIT 1
  ) p ON TRUE
)
SELECT
  ...
  product_id,
  neighborhood,
  '{nome} — {bairro} comprou hoje {produto}'::text AS template_text,
  ...
  12::integer AS purchase_minutes_ago,
...
WHERE NOT EXISTS (
  SELECT 1
  FROM store_notifications existing
  WHERE existing.type = _store_notification_seed.type
    AND existing.display_name = _store_notification_seed.display_name
    AND existing.product_id = _store_notification_seed.product_id
    AND existing.neighborhood = _store_notification_seed.neighborhood
    AND existing.start_date = _store_notification_seed.start_date
    AND existing.template_text = _store_notification_seed.template_text
)
...
WHERE EXISTS (
  SELECT 1
  FROM store_notifications inserted
  WHERE inserted.id = _store_notification_seed.id
)
ON CONFLICT (id) DO NOTHING;
