...
class AgenteWhatsAppMessage(Base):
    ...
    media_storage_key = Column(Text, nullable=True)
    ...
    quoted_provider_message_id = Column(String(255), nullable=True, index=True)
    response_to_message_id = Column(String, ForeignKey("agente_whatsapp_messages.id", ondelete="SET NULL"), nullable=True, index=True)
    campaign_id = Column(String, ForeignKey("whatsapp_campaigns.id", ondelete="SET NULL"), nullable=True, index=True)
    campaign_delivery_id = Column(String, ForeignKey("whatsapp_campaign_deliveries.id", ondelete="SET NULL"), nullable=True, index=True)
    provider_status = Column(String(40), nullable=True)
    processing_status = Column(String(30), nullable=False, default="recorded", index=True)
    idempotency_key = Column(String(220), nullable=True, unique=True)
    transcription_status = Column(String(30), nullable=False, default="none", index=True)
    transcription_text = Column(Text, nullable=True)
    transcription_language = Column(String(20), nullable=True)
    transcription_provider = Column(String(40), nullable=True)
    transcription_model = Column(String(120), nullable=True)
    transcription_error = Column(Text, nullable=True)
    transcription_quality_json = Column(Text, nullable=False, default="{}")
    ...
    processing_jobs = relationship("AgenteWhatsAppProcessingJob", back_populates="message", cascade="all, delete-orphan")
    audio_artifacts = relationship("AgenteWhatsAppAudioArtifact", back_populates="message", cascade="all, delete-orphan")
...
class AgenteWhatsAppAudioArtifact(Base):
    ...
    message = relationship("AgenteWhatsAppMessage", back_populates="audio_artifacts")
...
class AgenteWhatsAppProcessingJob(Base):
    ...
    message = relationship("AgenteWhatsAppMessage", back_populates="processing_jobs")
