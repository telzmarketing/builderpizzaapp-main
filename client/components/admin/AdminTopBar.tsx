...
export default function AdminTopBar() {
  ...
  const isAdmin = pathname.startsWith("/painel") && pathname !== "/painel/login";
  ...
  useEffect(() => {
    ...
    const fetch = () =>
      ordersApi
        .list({ limit: 20 })
        .then((orders) => setPendingOrders((orders ?? []).filter((o) => ACTIVE_STATUSES.has(o.status))))
        .catch(() => {});
    fetch();
  ...
  return (
    <div className="fixed top-3 right-4 z-50 flex items-center gap-1.5">
      ...
      <div className="flex items-center">
        {searchOpen ? (
          <form
            ...
            className="flex items-center gap-1.5 bg-surface-02 border border-surface-03 rounded-xl shadow-lg px-3 py-1.5"
          >
            ...
            <input
              ...
              className="bg-transparent text-cream text-sm w-44 outline-none placeholder:text-stone/50"
            />
            <button
              type="button"
          ...
          <button
            ...
            className="flex items-center justify-center w-9 h-9 rounded-xl bg-surface-02 border border-surface-03 text-stone hover:text-cream hover:border-surface-03/80 shadow-lg transition-colors"
            title="Buscar pedido"
          >
            <Search size={15} />
          </button>
      ...
      <button
        ...
        disabled={refreshing}
        className="flex items-center justify-center w-9 h-9 rounded-xl bg-surface-02 border border-surface-03 text-stone hover:text-cream shadow-lg transition-colors disabled:opacity-50"
      ...
      <div ref={bellRef} className="relative">
        <button
          ...
          className="relative flex items-center justify-center w-9 h-9 rounded-xl bg-surface-02 border border-surface-03 text-stone hover:text-cream shadow-lg transition-colors"
          title="Notificações"
        >
          <Bell size={15} />
          {count > 0 && (
        ...
        {bellOpen && (
          <div className="absolute top-11 right-0 w-80 bg-surface-02 border border-surface-03 rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-surface-03 flex items-center justify-between">
              ...
              <span className="text-[11px] text-stone">{count} pendente{count !== 1 ? "s" : ""}</span>
            </div>
