...
export default function StoreStatusBanner({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="rounded-xl border px-4 py-3 bg-red-500/10 border-red-500/30">
      <div className="flex items-start gap-3">
        <div className="mt-0.5 h-8 w-8 rounded-lg flex items-center justify-center bg-red-500/20 text-red-300">
          <Store size={16} />
        </div>
        <div className="min-w-0">
          <p className="text-sm font-bold text-red-300">{status.status_label}</p>
          {!compact && (
            <p className="text-stone text-xs mt-0.5">
              Hoje: {status.today_hours}
              {status.next_opening_label ? ` · Volta ${status.next_opening_label}` : ""}
            </p>
          )}
