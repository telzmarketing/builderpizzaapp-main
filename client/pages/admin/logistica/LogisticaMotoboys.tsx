...
import {
  Plus, Pencil, Loader2, Bike, Car, User, Footprints,
  CheckCircle, XCircle, Clock, Star, RefreshCw,
} from "lucide-react";
...
const BLANK: FormData = {
  name: "", phone: "", vehicle_type: "motorcycle",
  email: "", cpf: "", cnh: "", pix_key: "", password: "",
};
...
const STATUS_CONFIG: Record<string, { label: string; cls: string; icon: React.ReactNode }> = {
  available: { label: "Disponível", cls: "bg-green-500/15 text-green-300", icon: <CheckCircle size={12} /> },
...
function Inp({ label, name, value, onChange, type = "text", placeholder = "" }: {
...
export default function LogisticaMotoboys() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await deliveryApi.listPersons();
      setPersons(data ?? []);
    } catch {
  ...
  function openEdit(p: DeliveryPerson) {
    setForm({
      name: p.name, phone: p.phone, vehicle_type: p.vehicle_type,
      email: p.email ?? "", cpf: p.cpf ?? "", cnh: p.cnh ?? "",
      pix_key: p.pix_key ?? "", password: "",
    });
    setFormError("");
    setEditing(p);
    setModal("edit");
  ...
  async function handleSubmit() {
    ...
    try {
      const payload: Record<string, unknown> = {
        name: form.name, phone: form.phone, vehicle_type: form.vehicle_type,
        email: form.email || undefined, cpf: form.cpf || undefined,
        cnh: form.cnh || undefined, pix_key: form.pix_key || undefined,
      };
  ...
  async function deactivate(p: DeliveryPerson) {
    if (!confirm(`Desativar ${p.name}? Ele não poderá receber entregas.`)) return;
    try {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-stone text-sm mt-0.5">Cadastro e gestão dos entregadores</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {persons.map((p) => {
            ...
            return (
              <div key={p.id} className="rounded-2xl border border-surface-03 bg-surface-02 p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold flex-shrink-0 ${st.cls}`}>
                    {st.icon}
                    {st.label}
                  </span>
                </div>
                ...
                {p.email && (
                  <p className="mt-2 text-stone/70 text-xs truncate">{p.email}</p>
                )}
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  {p.status !== "busy" && (
                    ...
                    >
                      {p.status === "available" ? "Colocar Offline" : "Ativar"}
                    </button>
                  )}
                  <button
                    onClick={() => deactivate(p)}
                    className="rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-300 text-xs px-3 py-2 transition-colors"
                  >
                    <XCircle size={13} />
                  </button>
                </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-surface-02 rounded-2xl border border-surface-03 p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h3 className="text-cream font-bold text-lg mb-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Inp label="Senha" name="password" value={form.password} onChange={handleChange} type="password" placeholder={modal === "edit" ? "deixe em branco para manter" : ""} />
              <Inp label="CPF" name="cpf" value={form.cpf} onChange={handleChange} placeholder="000.000.000-00" />
              <Inp label="CNH" name="cnh" value={form.cnh} onChange={handleChange} placeholder="número da CNH" />
              <Inp label="Chave PIX" name="pix_key" value={form.pix_key} onChange={handleChange} placeholder="para repasse de valores" />

              <label className="flex flex-col gap-1">
                <span className="text-stone text-xs font-medium">Veículo</span>
                <select
                  name="vehicle_type" value={form.vehicle_type} onChange={handleChange}
