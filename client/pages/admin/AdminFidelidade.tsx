...
const colorPalette: Record<string, { label: string; color: string; bg: string; border: string; preview: string }> = {
  ...
  gray:   { label: "Prata",   color: "text-slate-300",  bg: "bg-slate-300/20",  border: "border-slate-300",  preview: "bg-slate-300"  },
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-950">
      <div className="flex h-screen">
        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-slate-800 px-8 py-4 border-b border-slate-700 sticky top-0 z-20">
            <h2 className="text-2xl font-bold text-white">Programa de Fidelidade</h2>
            <p className="text-slate-400 text-sm">Configure níveis, recompensas e regras de pontos</p>
          </div>
          ...
          <div className="p-8 space-y-10">
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-bold text-white">🏆 Níveis</h3>
                  <p className="text-slate-400 text-sm">Defina os níveis de fidelidade e a pontuação mínima de cada um</p>
                </div>
                <button
                  onClick={() => { setShowLevelForm(!showLevelForm); setEditingLevelId(null); setLevelForm(emptyLevel); }}
                  className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded-lg transition-colors text-sm"
                >
                  <Plus size={16} />
                  Novo Nível
                </button>
              ...
              {showLevelForm && (
                <form onSubmit={handleSaveLevel} className="bg-slate-800 rounded-xl p-5 border border-orange-500/40 mb-4 space-y-4">
                  <h4 className="text-white font-bold">{editingLevelId ? "Editar Nível" : "Novo Nível"}</h4>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="text" value={levelForm.name || ""} onChange={(e) => setLevelForm({ ...levelForm, name: e.target.value })}
                        className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" placeholder="Ex: Bronze" />
                    </div>
                    <div>
                      <label className="block text-slate-300 text-xs font-medium mb-1">Pontos mínimos *</label>
                      <input type="number" min="0" value={levelForm.minPoints ?? ""} onChange={(e) => setLevelForm({ ...levelForm, minPoints: parseInt(e.target.value) || 0 })}
                        className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" placeholder="0" />
                    </div>
                    <div>
                      <label className="block text-slate-300 text-xs font-medium mb-1">Ícone / Emoji</label>
                      <input type="text" value={levelForm.icon || ""} onChange={(e) => setLevelForm({ ...levelForm, icon: e.target.value })}
                        className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-2xl focus:outline-none focus:border-orange-500" placeholder="🥉" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-slate-300 text-xs font-medium mb-2">Cor do nível</label>
                    <div className="flex gap-2 flex-wrap">
                      {Object.entries(colorPalette).map(([key, val]) => (
                        <button key={key} type="button" onClick={() => setLevelForm({ ...levelForm, color: key })}
                          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm transition-colors ${levelForm.color === key ? "border-orange-500 bg-slate-700" : "border-slate-600 bg-slate-700/50 hover:border-slate-500"}`}>
                  ...
                  <div className="flex gap-3">
                    <button type="submit" className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded-lg transition-colors text-sm">
                      <Save size={14} />
                      {editingLevelId ? "Salvar" : "Criar Nível"}
                    </button>
                    <button type="button" onClick={() => { setShowLevelForm(false); setEditingLevelId(null); }}
                      className="px-4 py-2 rounded-lg border border-slate-600 text-slate-400 hover:border-slate-500 transition-colors text-sm">
                      Cancelar
                    </button>
                  </div>
                </form>
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-slate-400 text-sm">Prêmios que os clientes podem resgatar com seus pontos</p>
                </div>
                <button
              ...
              {showRewardForm && (
                <form onSubmit={handleSaveReward} className="bg-slate-800 rounded-xl p-5 border border-orange-500/40 mb-4 space-y-4">
                  <h4 className="text-white font-bold">{editingRewardId ? "Editar Recompensa" : "Nova Recompensa"}</h4>
                  <div className="grid grid-cols-3 gap-4">
                    <div className="col-span-2">
                      <label className="block text-slate-300 text-xs font-medium mb-1">Nome da recompensa *</label>
                      <input type="text" value={rewardForm.label || ""} onChange={(e) => setRewardForm({ ...rewardForm, label: e.target.value })}
                        className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" placeholder="Ex: 10% de desconto" />
                    </div>
                    <div>
                      <label className="block text-slate-300 text-xs font-medium mb-1">Ícone / Emoji</label>
                      <input type="text" value={rewardForm.icon || ""} onChange={(e) => setRewardForm({ ...rewardForm, icon: e.target.value })}
                        className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-2xl focus:outline-none focus:border-orange-500" placeholder="🏷️" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-slate-300 text-xs font-medium mb-1">Pontos necessários *</label>
                    <input type="number" min="1" value={rewardForm.points ?? ""} onChange={(e) => setRewardForm({ ...rewardForm, points: parseInt(e.target.value) || 0 })}
                      className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" placeholder="300" />
                  </div>
                  <div className="flex gap-3">
                    <button type="submit" className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded-lg transition-colors text-sm">
                      <Save size={14} />
              ...
              <div className="grid grid-cols-2 gap-3">
                {sortedRewards.map((reward) => (
                  <div key={reward.id} className="bg-slate-800 rounded-xl p-4 border border-slate-700 flex items-center gap-3">
                    <span className="text-3xl">{reward.icon}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-sm font-bold truncate">{reward.label}</p>
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-bold text-white">⚡ Como ganhar pontos</h3>
                  <p className="text-slate-400 text-sm">Regras que explicam aos clientes como acumular pontos</p>
                </div>
                <button
              ...
              {showRuleForm && (
                <form onSubmit={handleSaveRule} className="bg-slate-800 rounded-xl p-5 border border-orange-500/40 mb-4 space-y-4">
                  <h4 className="text-white font-bold">{editingRuleId ? "Editar Regra" : "Nova Regra"}</h4>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-slate-300 text-xs font-medium mb-1">Ícone / Emoji</label>
                    ...
                    <div className="col-span-2">
                      ...
                      <input type="text" value={ruleForm.label || ""} onChange={(e) => setRuleForm({ ...ruleForm, label: e.target.value })}
                        className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" placeholder="Ex: A cada R$1 gasto" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-slate-300 text-xs font-medium mb-1">Pontos ganhos</label>
                    <input type="text" value={ruleForm.points || ""} onChange={(e) => setRuleForm({ ...ruleForm, points: e.target.value })}
                      className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" placeholder="+10 pts" />
                  </div>
                  <div className="flex gap-3">
                    <button type="submit" className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded-lg transition-colors text-sm">
                      <Save size={14} />
              ...
              <div className="space-y-3">
                {earnRules.map((rule) => (
                  <div key={rule.id} className="flex items-center gap-4 bg-slate-800 rounded-xl p-4 border border-slate-700">
                    <span className="text-2xl">{rule.icon}</span>
                    <p className="text-white text-sm flex-1">{rule.label}</p>
