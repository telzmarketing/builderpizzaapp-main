        ...
        <div className="flex-1 overflow-auto">
          <div className="bg-slate-800 px-8 py-4 border-b border-slate-700 flex justify-between items-center sticky top-0 z-20">
            <div>
              ...
              <p className="text-slate-400 text-sm">{coupons.filter((c) => !c.used).length} disponíveis · {coupons.filter((c) => c.used).length} usados</p>
            </div>
            <button
          ...
          <div className="p-8">
            {showForm && (
              <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 mb-8">
                <h3 className="text-xl font-bold text-white mb-4">{editingId ? "Editar Cupom" : "Novo Cupom"}</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-slate-300 text-sm font-medium mb-2">Código do Cupom *</label>
                      <input
                        type="text"
                        value={formData.code || ""}
                        onChange={(e) => setFormData({ ...formData, code: e.target.value.toUpperCase() })}
                        className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white placeholder-slate-400 focus:outline-none focus:border-orange-500 font-mono tracking-widest uppercase"
                        placeholder="EX: PIZZA20"
                      />
                    </div>
                    <div>
                      <label className="block text-slate-300 text-sm font-medium mb-2">Tipo de Desconto *</label>
                      <select
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-slate-300 text-sm font-medium mb-2">Texto do Desconto *</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-slate-300 text-sm font-medium mb-2">Validade *</label>
                      <input
                        type="text"
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      id="used-check"
                    />
                    <label htmlFor="used-check" className="text-slate-300 text-sm font-medium cursor-pointer">Marcar como já utilizado</label>
                  </div>
            ...
            <div className="flex gap-2 mb-6">
              {(["all", "available", "used"] as const).map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${filter === f ? "bg-orange-500 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700 border border-slate-700"}`}
                >
            ...
            <div className="space-y-4">
              {filtered.map((coupon) => (
                <div key={coupon.id} className={`bg-slate-800 rounded-xl border border-slate-700 overflow-hidden ${coupon.used ? "opacity-60" : ""}`}>
                  <div className="flex items-center gap-4 p-4">
                    <div className="w-14 h-14 rounded-full bg-slate-700 flex items-center justify-center text-3xl flex-shrink-0">
                      {coupon.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-bold">{coupon.description}</p>
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs border ${typeColor(coupon.type)}`}>
                          {coupon.discount}
                        </span>
                        <span className="text-slate-400 text-xs font-mono tracking-widest">{coupon.code}</span>
                        <span className="text-slate-500 text-xs">Válido até {coupon.expiry}</span>
                        {coupon.used && <span className="text-xs bg-slate-700 text-slate-400 px-2 py-0.5 rounded-full">Usado</span>}
                      </div>
                    </div>
                    <div className="flex gap-2 flex-shrink-0">
                      <button
                        onClick={() => updateCoupon(coupon.id, { used: !coupon.used })}
                        className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors ${coupon.used ? "bg-green-500/20 text-green-400 hover:bg-green-500/30" : "bg-slate-700 text-slate-300 hover:bg-slate-600"}`}
                      >
            ...
            {filtered.length === 0 && (
              <div className="text-center py-16">
                ...
                <p className="text-slate-400 text-lg">Nenhum cupom aqui</p>
