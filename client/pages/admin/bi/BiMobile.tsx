...
const STATUS_ITEMS: Array<{ key: ApiBIMobileStatusKey; label: string; icon: ElementType }> = [
  ...
  { key: "pronto_para_entrega", label: "Pronto para entrega", icon: ShoppingBag },
  ...
  return (
    <div className="min-h-[104px] rounded-lg border border-surface-03 bg-surface-02 p-3">
      <div className="flex items-start justify-between gap-2">
        <p className="text-[11px] font-bold uppercase leading-snug text-stone">{item.label}</p>
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gold/10 text-gold">
          <Icon size={16} />
        </span>
      </div>
      <p className="mt-3 break-words text-xl font-black leading-tight text-cream">{item.value}</p>
      {item.helper && <p className="mt-1 text-[11px] leading-snug text-stone">{item.helper}</p>}
    </div>
  );
}
...
export default function BiMobile() {
  ...
  const maxStatusCount = Math.max(...STATUS_ITEMS.map((item) => data?.ordersByStatus[item.key] ?? 0), 1);
    ...
    <div className="mx-auto flex w-full max-w-md flex-col gap-4 pb-6">
      <section className="rounded-lg border border-surface-03 bg-surface-02 p-3">
        <div className="flex items-center gap-2">
          ...
          <label className="text-xs font-bold uppercase text-stone" htmlFor="bi-mobile-date">
            Data
          </label>
        </div>
        ...
        <>
          ...
          <section className="rounded-lg border border-surface-03 bg-surface-02 p-3">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-sm font-black text-cream">Status dos pedidos</h2>
            ...
            {!hasOrders ? (
              ...
              <div className="mt-3 space-y-2">
                {STATUS_ITEMS.map((item) => {
                  ...
                  return (
                    <div key={item.key} className="rounded-lg border border-surface-03 bg-surface-01 p-2.5">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="text-sm font-black text-cream">{number(total)}</span>
                      </div>
